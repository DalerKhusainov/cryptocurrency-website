// ICONS
import { HiChartBar, HiUser, HiGlobe } from "react-icons/hi";

const Stats = () => {
  return (
    <section className="py-24">
      <div
        className="container mx-auto"
        data-aos="fade-up"
        data-aos-delay="1200"
      >
        <div className="flex flex-col gap-y-6 lg:flex-row lg:justify-between">
          {/* ITEM */}
          <div className="flex items-center gap-x-6">
            {/* ITEM ICON */}
            <div className="flex items-center justify-center w-20 h-20 text-2xl rounded-full bg-white/10 text-blue lg:text-4xl">
              <HiChartBar />
            </div>
            {/* ITEM TEXT */}
            <div>
              <div className="text-4xl font-bold lg:text-[40px] lg:mb-2">
                $30B
              </div>
              <div className="text-gray">Digital Currency Exchanged</div>
            </div>
          </div>
          {/* ITEM */}
          <div className="flex items-center gap-x-6">
            {/* ITEM ICON */}
            <div className="flex items-center justify-center w-20 h-20 text-2xl rounded-full bg-white/10 text-blue lg:text-4xl">
              <HiUser />
            </div>
            {/* ITEM TEXT */}
            <div>
              <div className="text-4xl font-bold lg:text-[40px] lg:mb-2">
                $10M+
              </div>
              <div className="text-gray">Trusted Wallets Investor</div>
            </div>
          </div>
          {/* ITEM */}
          <div className="flex items-center gap-x-6">
            {/* ITEM ICON */}
            <div className="flex items-center justify-center w-20 h-20 text-2xl rounded-full bg-white/10 text-blue lg:text-4xl">
              <HiGlobe />
            </div>
            {/* ITEM TEXT */}
            <div>
              <div className="text-4xl font-bold lg:text-[40px] lg:mb-2">
                195
              </div>
              <div className="text-gray">Countries Supported</div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Stats;
